from articles.models import User,Places,Trip,PlacesInTrip,UserTrips
...
def check_in(place_name,trip_id):
		...
		t=Places.objects.filter(place_name=place_name)
...
def display_trip_details(trip_id):
	try:
		...
		raise Exception("Couldn't return trip details of the trip with id : "+ str(trip_id))
	...
	try:
		...
		if c:
			...
			for i in c:
				t=i.trip_id.trip_id
				t1 = str(t)
				d[t1]=Trip.objects.get(trip_id=t).trip_review
			return d
	except:
		raise Exception("No user with : "+str(phone_number));
