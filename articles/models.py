...
def send_message_to_group(phone_number, g_id, video_url=None, photo_url=None, text=None):
    ...
    try:
        messageEntry = GroupMessage.objects.create(
            video_url=video_url, photo_url=photo_url, text=text)
    except:
        raise Exception("Error during message creation")

    try:
        UserSendsGroupMessage.objects.create(
            phone_number=phone_number, g_id=g_id, gm_id=messageEntry.gm_id)
    ...
    try:
        for tableEntry in Entry.objects.filter(g_id=g_id):
            UserReceivesGroupMessage.objects.create(
                phone_number=tableEntry.phone_number, g_id=g_id, gm_id=messageEntry.gm_id)
